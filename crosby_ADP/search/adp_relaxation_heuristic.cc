...
void ADPRelaxationHeuristic::initialize() {
    ...
    if(no_of_agents > 1){
        ...
        cout << agent_joint_operators.size() << " 22agents** left!" << endl;
    }
    else{
    ...
    if(no_of_agents > 1){
        ...
        agent_operators_.resize(no_of_agents);
        agent_operators.resize(no_of_agents);
        agent_joint_operators.resize(no_of_agents);

        //creating original operator sets
        for (int i = 0; i < all_operators.size(); i += 1)
        {
            OperatorProxy op = all_operators[i];
            std::set< int> used_by;
            vector<ADP_Proposition *> precondition_props = vector<ADP_Proposition*>();
            for (FactProxy precondition : op.get_preconditions()) 
            {
                if(get_proposition(precondition)->agent_id != -1)
                    used_by.insert(get_proposition(precondition)->agent_id);
            }
            for (EffectProxy effect : op.get_effects()) {
                ...
                for (FactProxy eff_cond : eff_conds) {
                    if(get_proposition(eff_cond)->agent_id != -1)
                        used_by.insert(get_proposition(eff_cond)->agent_id);
                }
            }
            if(used_by.size() == 0){
                public_operators_.push_back(op);
            }            
            else if (used_by.size() == 1)
            {
                agent_operators_[*used_by.begin()].push_back(op);
            }
            else
            {
                cout << "JOINTTTTT " << used_by.size() << endl;
            }
        }
    ...
    //cout << "RESULTS " << all_operators.size() << " " << public_operators_.size() << " " << unary_operators.size() << endl;
    //cout << "HAJI1 " + no_of_agents << "\n \n";
    if (no_of_agents > 1)
    {
        for (int i = 0; i < no_of_agents; i += 1)
            cout << i << " " << agent_operators_[i].size() << endl;

		int max_ops = 10;
		ofstream file;
	  	file.open ("out/agents.txt");
		  
		VariablesProxy p = task_proxy.get_variables();
		OperatorsProxy op = task_proxy.get_operators();
	
		file << "public variables: \n";
	
		int s = public_vars.size();
    	file << "   " <<  s << "\n";
		for (int k = 0; k < (s <= max_ops ? s : max_ops); k++)
		{
			int pvar = public_vars[k];
			int i = 0;
			file << "   " << k << "_";
			while (p[pvar].get_name()[i] > 0)
				file << p[pvar].get_name()[i++];
			file << '\n';
			file << "      axioms: \n";
			for (int l = 0; l < p[pvar].get_domain_size(); l++)
			{
				FactProxy fact = p[pvar].get_fact(l);
				int i = 0;
				file << "      ";
				while (fact.get_name()[i] > 0)
					file <<  fact.get_name()[i++];
				file << '\n';
			}
		}
		file << "public operators: \n";
		s = public_operators.size();
    	file << "   " <<  s << "\n";
		for (int k = 0; k < (s <= max_ops ? s : max_ops); k++)
		{
			ADP_UnaryOperator *uop = public_operators[k];
			int i = 0;
			file << "   " << k << "_";
			while (op[uop->operator_no].get_name()[i] > 0)
				file << op[uop->operator_no].get_name()[i++];
			file <<  "\n";
		}
	
		for (int j = 0; j < agent_vars.size(); j++)
		{
			file << "------------------------------Agent " << j << "------------------------------ \n";
			file << "variables: \n";
			file << "   " << agent_vars[j].size() << "\n";
			for (int k = 0; k < agent_vars[j].size(); k++)
			{
				int i = 0;
				while (p[agent_vars[j][k]].get_name()[i] > 0)
					file <<  p[agent_vars[j][k]].get_name()[i++];
				
				file << '\n';
				file << "   axioms: \n";
				for (int l = 0; l < p[agent_vars[j][k]].get_domain_size(); l++)
				{
					FactProxy fact = p[agent_vars[j][k]].get_fact(l);
					int i = 0;
					file << "   ";
					while (fact.get_name()[i] > 0)
						file <<  fact.get_name()[i++];
			...
			file << "operators: \n";
			s = agent_operators[j].size();
			file << "   " << s << "\n";
			for (int k = 0; k < (s <= max_ops ? s : max_ops); k++)
			{
				ADP_UnaryOperator *uop = agent_operators[j][k];
				int i = 0;
				file << "   " << k << "_";
				while (op[uop->operator_no].get_name()[i] > 0)
					file << op[uop->operator_no].get_name()[i++];
...
bool ADPRelaxationHeuristic::vector_has_operator (vector<OperatorProxy> vector, OperatorProxy op)
{
    for (size_t i = 0; i < vector.size(); i += 1)
    {
        if (vector[i].get_id() == op.get_id())
            return true;
    }
    return false;
}
